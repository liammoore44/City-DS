...
def parameters():
    ...
    params['kappa_def'] =  1.72 # kappa parameter in Spearman 2018 that gives the advantage defending players to control ball
...
def where_home_team_attacks(home_tracking):
    '''
    ...
    if home_tracking.loc[game_start_frame, home_team_x_cols].mean() > 0:
        return (-1)
    else:
        return (1)
...
def generate_pitch_control_for_frame(frame, tracking_home, tracking_away,
                                     attacking_team, params,
                                     field_dimen=(106., 68.,),
                                     n_grid_cells_x=50):
    ...
    ball_start_pos = np.array([tracking_home.loc[frame]['ball_x'],
                               tracking_home.loc[frame]['ball_y']])
    ...
    if attacking_team == 'Home':
        attacking_players = initialise_players(tracking_home.loc[frame], 'Home',
                                               params)
        defending_players = initialise_players(tracking_away.loc[frame], 'Away',
                                               params)
    else:
        defending_players = initialise_players(tracking_home.loc[frame], 'Home',
                                               params)
        attacking_players = initialise_players(tracking_away.loc[frame], 'Away',
                                               params)
    ...
    # assert abs(1-checksum) < params['model_converge_tol'],\
