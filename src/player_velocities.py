...
def calc_player_acceleration(team, smoothing=True, filter_='moving average', window=7, polyorder=1, maxspeed = 12, max_acc=12):
    """ calc_player_acceleration( tracking_data )

    Calculate player acceleration in x & y direciton, and total player acceleration at each timestamp of the tracking data

    Parameters
    -----------
        team: the tracking DataFrame for home or away team
        smoothing: boolean variable that determines whether acceleration measures are smoothed. Default is True.
    ...
    Returns
    -----------
       team : the tracking DataFrame with columns for acceleration in the x & y direction and total acceleration added
    """
    # remove any velocity data already in the dataframe
    team = remove_player_velocities(team)
    ...
    team= calc_player_velocities(team, smoothing=smoothing, filter_=filter_,
                                 window=window, polyorder=polyorder, maxspeed=maxspeed)
    ...
    # estimate acceleration for players in team
    for player in player_ids:  # cycle through players individually
        # difference player speeds in timestep dt to get unsmoothed estimate of acceleration
        ax = team[player+"_vx"].diff() / dt
        ay = team[player+"_vy"].diff() / dt

        if max_acc > 0:
            # remove unsmoothed data points that exceed the maximum acceleration (these are most likely position errors)
            raw_speed = np.sqrt(ax**2 + ay**2)
            ax[raw_speed>max_acc] = np.nan
            ay[raw_speed>max_acc] = np.nan
